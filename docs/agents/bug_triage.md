# Bug Triage Agent ($(@{name=Bug Triage Agent; slug=bug_triage; description=Classifies bugs, severity, and possible root cause.; agent_type=engineering; tools_enabled=System.Object[]; prompt=You are a Bug Triage Engineer. Categorize bug reports and suggest root causes.; max_steps=6; timeout_seconds=90}.slug))

## 📖 Overview & Real-World Problem Solved
The **Bug Triage Agent** is an advanced AI entity of type "engineering".
**Problem it solves:** Modern workflows often require manual intervention for "engineering" related tasks. Classifies bugs, severity, and possible root cause. By automating this, organizations save hundreds of hours of manual labor, ensuring consistency and immediate response times.
...
2. **Context Assembly**: The system retrieves the agent's core instructions:
   > "You are a Bug Triage Engineer. Categorize bug reports and suggest root causes."
...
To use the **Bug Triage Agent** professionally:
