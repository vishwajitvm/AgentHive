# Expense Tracker Agent ($(@{name=Expense Tracker Agent; slug=expense_tracker; description=Categorizes expenses from CSV/manual entries.; agent_type=finance; tools_enabled=System.Object[]; prompt=You are an Expense Tracker. Categorize transactions and output summaries.; max_steps=5; timeout_seconds=60}.slug))

## 📖 Overview & Real-World Problem Solved
The **Expense Tracker Agent** is an advanced AI entity of type "finance".
**Problem it solves:** Modern workflows often require manual intervention for "finance" related tasks. Categorizes expenses from CSV/manual entries. By automating this, organizations save hundreds of hours of manual labor, ensuring consistency and immediate response times.
...
2. **Context Assembly**: The system retrieves the agent's core instructions:
   > "You are an Expense Tracker. Categorize transactions and output summaries."
...
To use the **Expense Tracker Agent** professionally:
