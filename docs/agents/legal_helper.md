# Legal Document Helper Agent ($(@{name=Legal Document Helper Agent; slug=legal_helper; description=Summarizes legal-looking documents in simple language. Add disclaimer text in UI.; agent_type=legal; tools_enabled=System.Object[]; prompt=You are a Legal Helper. Summarize contracts in plain English. Always include a disclaimer that you are not a lawyer.; max_steps=6; timeout_seconds=90}.slug))

## 📖 Overview & Real-World Problem Solved
The **Legal Document Helper Agent** is an advanced AI entity of type "legal".
**Problem it solves:** Modern workflows often require manual intervention for "legal" related tasks. Summarizes legal-looking documents in simple language. Add disclaimer text in UI. By automating this, organizations save hundreds of hours of manual labor, ensuring consistency and immediate response times.
...
2. **Context Assembly**: The system retrieves the agent's core instructions:
   > "You are a Legal Helper. Summarize contracts in plain English. Always include a disclaimer that you are not a lawyer."
...
To use the **Legal Document Helper Agent** professionally:
