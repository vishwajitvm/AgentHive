# GitHub Tool ($(@{name=GitHub Tool; slug=github_tool; description=Reads repos, issues, PRs, and commits.; category=development; input_schema=; output_schema=; requires_auth=True; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **GitHub Tool** is a backend utility belonging to the "development" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Reads repos, issues, PRs, and commits. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
`json
{"action":"string","repo":"string","issue_title":"string"}
``n
...
*Synergy:* Tools in the "development" category pair exceptionally well with reporting agents (to write results) or analytical agents (to read results).
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to github_tool" to save costs.
