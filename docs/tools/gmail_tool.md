# Gmail Tool ($(@{name=Gmail Tool; slug=gmail_tool; description=Reads/sends Gmail through official OAuth architecture or mock mode.; category=communication; input_schema=; output_schema=; requires_auth=True; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **Gmail Tool** is a backend utility belonging to the "communication" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Reads/sends Gmail through official OAuth architecture or mock mode. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
`json
{"action":"string","to":"string","subject":"string","body":"string"}
``n
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to gmail_tool" to save costs.
