# OCR Tool ($(@{name=OCR Tool; slug=ocr_tool; description=Optional text extraction from images, disabled by default if dependency missing.; category=vision; input_schema=; output_schema=; requires_auth=False; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **OCR Tool** is a backend utility belonging to the "vision" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Optional text extraction from images, disabled by default if dependency missing. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to ocr_tool" to save costs.
