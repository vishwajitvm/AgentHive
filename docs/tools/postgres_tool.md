# PostgreSQL Query Tool ($(@{name=PostgreSQL Query Tool; slug=postgres_tool; description=Runs safe internal queries only.; category=database; input_schema=; output_schema=; requires_auth=True; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **PostgreSQL Query Tool** is a backend utility belonging to the "database" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Runs safe internal queries only. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
*Synergy:* Tools in the "database" category pair exceptionally well with reporting agents (to write results) or analytical agents (to read results).
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to postgres_tool" to save costs.
