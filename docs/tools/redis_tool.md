# Redis Cache Tool ($(@{name=Redis Cache Tool; slug=redis_tool; description=Reads/writes temporary cached values.; category=database; input_schema=; output_schema=; requires_auth=True; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **Redis Cache Tool** is a backend utility belonging to the "database" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Reads/writes temporary cached values. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to redis_tool" to save costs.
