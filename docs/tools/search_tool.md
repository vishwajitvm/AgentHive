# Web Search Tool ($(@{name=Web Search Tool; slug=search_tool; description=Searches web using provider abstraction or mock mode.; category=core; input_schema=; output_schema=; requires_auth=False; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **Web Search Tool** is a backend utility belonging to the "core" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Searches web using provider abstraction or mock mode. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to search_tool" to save costs.
