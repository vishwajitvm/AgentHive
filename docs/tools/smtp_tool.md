# Email SMTP Tool ($(@{name=Email SMTP Tool; slug=smtp_tool; description=Sends emails through configured SMTP.; category=communication; input_schema=; output_schema=; requires_auth=True; required_env_keys=System.Object[]; safe_mock_mode=True}.slug))

## 📖 Overview & Real-World Problem Solved
The **Email SMTP Tool** is a backend utility belonging to the "communication" category.
**Problem it solves:** LLMs natively cannot interact with the outside world. Sends emails through configured SMTP. By exposing this tool to the AI, we bridge the gap between static text generation and dynamic, real-world execution.
...
2. If this tool interacts with rate-limited external APIs, ensure your Agent's prompt explicitly instructs it to "Minimize calls to smtp_tool" to save costs.
